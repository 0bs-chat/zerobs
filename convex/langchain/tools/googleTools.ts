...
async function getGoogleAccessToken(config: ExtendedRunnableConfig) {
  const externalAccount = await clerkClient.users.getUserOauthAccessToken(config.chat.userId, "oauth_custom_google_poogle");
  return externalAccount.data.length > 0 ? externalAccount.data[0].token : undefined;
}
...
export const getGoogleTools = async (
  ...
  const listCalendarseTool = new DynamicStructuredTool({
    ...
    description: "List all Google Calendars accessible to the user. Use this to see available calendars before working with events.",
    schema: z.object({}),
    func: async () => {
      try {
        const result = await makeGoogleAPIRequest("/users/me/calendarList", accessToken);
  ...
  const listCalendarEventsTool = new DynamicStructuredTool({
    ...
    description: "List events from a specific Google Calendar. Use this to see upcoming or past events.",
    schema: z.object({
      calendarId: z.string().describe("The calendar ID (use 'primary' for default calendar)").default("primary"),
      timeMin: z.string().optional().describe("Lower bound (inclusive) for an event's end time (RFC3339 timestamp)"),
      timeMax: z.string().optional().describe("Upper bound (exclusive) for an event's start time (RFC3339 timestamp)"),
      maxResults: z.number().min(1).max(2500).default(10).describe("Maximum number of events to return"),
      q: z.string().optional().describe("Free text search terms to find events"),
    }),
    func: async ({ calendarId = "primary", timeMin, timeMax, maxResults = 10, q }) => {
      try {
  ...
  const createCalendarEventTool = new DynamicStructuredTool({
    ...
    description: "Create a new event in a Google Calendar. Use this to schedule meetings, appointments, or reminders.",
    schema: z.object({
      calendarId: z.string().describe("The calendar ID (use 'primary' for default calendar)").default("primary"),
      ...
      startDateTime: z.string().describe("Start date and time (RFC3339 format, e.g., '2024-01-15T09:00:00-07:00')"),
      endDateTime: z.string().describe("End date and time (RFC3339 format, e.g., '2024-01-15T10:00:00-07:00')"),
      location: z.string().optional().describe("The location of the event"),
      attendees: z.array(z.string()).optional().describe("List of email addresses of attendees"),
    }),
    func: async ({ calendarId = "primary", summary, description, startDateTime, endDateTime, location, attendees }) => {
      try {
        const event = {
          summary,
          description,
  ...
  const updateCalendarEventTool = new DynamicStructuredTool({
    ...
    description: "Update an existing event in a Google Calendar. Use this to modify event details.",
    schema: z.object({
      calendarId: z.string().describe("The calendar ID (use 'primary' for default calendar)").default("primary"),
      ...
      startDateTime: z.string().optional().describe("New start date and time (RFC3339 format)"),
      endDateTime: z.string().optional().describe("New end date and time (RFC3339 format)"),
      location: z.string().optional().describe("The new location of the event"),
    }),
    func: async ({ calendarId = "primary", eventId, summary, description, startDateTime, endDateTime, location }) => {
      try {
  ...
  const deleteCalendarEventTool = new DynamicStructuredTool({
    ...
    description: "Delete an event from a Google Calendar. Use this to cancel or remove events.",
    schema: z.object({
      calendarId: z.string().describe("The calendar ID (use 'primary' for default calendar)").default("primary"),
    ...
    func: async ({ calendarId = "primary", eventId }) => {
      try {
        ...
        const result = { success: true, message: `Event ${eventId} deleted successfully` };
  ...
  const listGmailMessagesTool = new DynamicStructuredTool({
    ...
    description: "List Gmail messages. Use this to search and retrieve email messages.",
    schema: z.object({
  ...
  const getGmailMessageTool = new DynamicStructuredTool({
    ...
    description: "Get the full content of a specific Gmail message. Use this to read the complete email content.",
    schema: z.object({
    ...
    func: async ({ messageId, format = "full" }) => {
      try {
        ...
        if (result.payload?.body?.data) {
          ...
          // Multi-part message
          const textPart = result.payload.parts.find((part: any) => part.mimeType === "text/plain");
  ...
  const sendGmailMessageTool = new DynamicStructuredTool({
    ...
    description: "Send a new Gmail message. Use this to compose and send emails.",
    schema: z.object({
  ...
  const searchGmailTool = new DynamicStructuredTool({
    name: "searchGmail",
    description: "Search Gmail messages with advanced query options. Use this for complex email searches.",
    schema: z.object({
      query: z.string().describe("Gmail search query (supports Gmail search operators like 'from:', 'subject:', 'has:attachment', etc.)"),
    ...
    func: async ({ query, maxResults = 10 }) => {
      try {
        ...
        const messages = await Promise.all(
          (result.messages || []).slice(0, maxResults).map(async (message: any) => {
            const fullMessage = await makeGmailAPIRequest(
