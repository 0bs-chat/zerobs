...
export const getMCPTools = async (
	...
	if (mcps.length === 0) {
		return {
			tools: [],
			groupedTools: {} as Record<string, DynamicStructuredTool[]>,
		};
	}
	...
	const clientsAndTools = await Promise.all(
		mcps.map(async (mcp) => {
			try {
				const mcpConfigurableEnvs = await resolveConfigurableEnvs(
					ctx,
					mcp,
					true,
				);
				...
				if (mcp.perChat && ["stdio", "docker"].includes(mcp.type) && config) {
					try {
						appDoc = await ctx.runMutation(
							internal.mcps.mutations.assignAppToChat,
							{
								mcpId: mcp._id,
						...
						console.error(
							`Failed to assign per-chat MCP machine for ${mcp.name}:`,
							error,
						);
						return null;
					}
				} else {
					appDoc = mcp.apps?.[0] ?? null;
				}
	...
	const validClientsAndTools = clientsAndTools.filter(
	...
	const allTools = validClientsAndTools.flatMap(({ tools }) => tools);

	// File upload logic for MCPs if state/messages present
	if (state && state.messages && state.messages.length > 0) {
		...
		await Promise.all(
			validClientsAndTools.map(async ({ mcp, appDoc, machine }) => {
				if (
					["stdio", "docker"].includes(mcp.type) &&
					files.length > 0 &&
					machine?.id
				) {
	...
	// Group tools by MCP name for groupedTools
	const groupedTools = new Map<string, DynamicStructuredTool[]>();
	for (const { mcp, tools } of validClientsAndTools) {
		groupedTools.set(mcp.name, tools);
	}
