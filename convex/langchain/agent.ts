...
async function shouldRetrieve(
  ...
  if (formattedConfig.chat.projectId || formattedConfig.chat.webSearch) {
    return "true";
  }
...
async function retrieve(
    ...
    await Promise.all(
      queries.queries.map(async (query) => {
        ...
        if (Array.isArray(searchResults)) {
          documents.push(
            ...(searchResults.filter((doc) => doc !== null) as Document[])
          );
        }
      })
    );
  ...
  if (formattedConfig.chat.webSearch) {
    const queries = await generateQueries(state, formattedConfig, "webSearch");
...
async function simple(state: typeof GraphState.State, config: RunnableConfig) {
  ...
  const baseResponseWithDocuments = [
    addDocumentsToMessage(response, state.documents),
  ];
