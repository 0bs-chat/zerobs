...
export const AGENT_SETTINGS = [
	{
		...
		detail:
			"Generate code, documents, visualizations, and interactive apps as standalone artifacts that can be edited and updated.",
	...
	},
	{
		...
		detail:
			"Access current information, news, research papers, and technical documentation beyond the AI's knowledge cutoff.",
	...
	},
	{
		...
		detail:
			"Coordinates multiple AI agents, each equipped with specific MCP tool sets, using a supervisor pattern for complex workflows.",
	...
	},
	{
		...
		detail:
			"Breaks down complex requests into structured step-by-step plans, executes them systematically, and adapts based on results.",
...
const AgentSettingItem = ({
	setting,
	isEnabled,
	onToggle,
}: AgentSettingItemProps) => {
	const IconComponent = setting.icon;
	const hasDependencies = setting.dependencies.length > 0;

	return (
		<button
			type="button"
			className={`w-full flex flex-col gap-3 px-4 py-4 text-left rounded-lg transition-all duration-200 hover:bg-accent/25 dark:hover:bg-accent/60 border-2 focus:outline-none ${
				isEnabled
		...
		>
			...
			<div
				className="flex gap-1.5 items-start justify-between"
				id={`${setting.key}-description`}
			>
				<p className="text-xs text-foreground/70 leading-relaxed mr-2">
					{setting.description}
				</p>
				<Tooltip>
					<TooltipTrigger asChild>
						<button
							type="button"
							className="flex items-center gap-1 text-xs text-muted-foreground/70 hover:text-foreground transition-colors focus:outline-none bg-card rounded cursor-pointer"
							onClick={(e) => e.stopPropagation()}
							tabIndex={-1}
							aria-label={`More details about ${setting.label}`}
						>
					...
					<div className="flex items-center justify-between">
						<div className="flex flex-col">
							...
							<p className="text-xs text-muted-foreground">
								Configure AI capabilities and behaviors
							</p>
						</div>
					</div>
				</div>
