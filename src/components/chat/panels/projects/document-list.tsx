...
export function ProjectDocumentList({ projectId }: { projectId: Id<"projects"> }) {
    ...
    <div className="flex flex-col gap-2 bg-card rounded-md shadow-sm border p-4">
      <div className="flex items-center px-3 gap-3">
        ...
        <label htmlFor="select-all" className="text-sm text-muted-foreground">
          Select All ({projectDocuments.projectDocuments.filter(projectDocument => projectDocument.selected).length}/{projectDocuments.projectDocuments.length})
        </label>
      </div>
