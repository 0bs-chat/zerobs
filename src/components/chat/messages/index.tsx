...
export const ChatMessages = ({ chatId }: { chatId: Id<"chats"> | "new" }) => {
	...
	const mainContent = useMemo(() => {
		if (chatId === "new") {
			...
			return (
				<div
					className={`flex items-center justify-center h-full flex-col gap-4 ${isMobile && resizePanelOpen ? "hidden" : "flex"} -translate-y-12`}
				>
					<div
						className="flex items-center gap-2 sm:text-5xl text-2xl font-semibold text-primary/50"
						style={{
							fontFamily: "Rubik",
						}}
					>
