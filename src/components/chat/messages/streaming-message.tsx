    ...
    if (tool.toolName === "searchWeb") {
      try {
        ...
        if (typeof tool.output === "object" && tool.output !== null) {
          ...
          if (
            "kwargs" in tool.output &&
            typeof tool.output.kwargs === "object" &&
            tool.output.kwargs !== null &&
            "content" in tool.output.kwargs
          ) {
        ...
        console.error(
          "Failed to parse search results from streaming tool output",
          err
        );
...
export const StreamingMessage = memo(
  ({ stream }: { stream: ReturnType<typeof useStream> }) => {
    ...
    const renderedChunks = useMemo(() => {
      ...
      return stream.chunkGroups.map((chunk, index) => {
        if (chunk.type === "ai") {
          ...
          return <StreamingToolCall key={`tool-${index}`} tool={chunk} />;
        }
        return null;
      });
    }, [stream?.chunkGroups]);
    ...
    return <div className="space-y-3">{renderedChunks}</div>;
  }
);
