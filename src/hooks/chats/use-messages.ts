...
export const useNavigateBranch = () => {
	return useCallback((_depth: number, _direction: "prev" | "next" | number) => {
		// No-op: linear processing doesn't need branch navigation
		console.warn(
			"Branch navigation is no longer supported with optimized linear processing",
		);
	}, []);
};

export const useChangeBranch = () => {
	return useCallback((_depth: number, _newBranchIndex: number) => {
		// No-op: linear processing doesn't need branch changes
		console.warn(
			"Branch changing is no longer supported with optimized linear processing",
		);
	}, []);
};
