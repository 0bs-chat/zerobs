...
async function fetchWebPageWithPuppeteerGhost(targetUrl: string): Promise<{ html: string; status: number; contentType?: string }> {
...
function parseWebPageWithDOM(html: string, targetUrl: string, status: number, contentType?: string) {
  ...
  if (returnContent) {
    ...
    const mainContent = doc.querySelector('main, article, .content, .post, .entry, [role="main"]')?.textContent || 
  ...
  if (returnMetadata) {
    ...
    metadata = {
      ...
      language: getMetaContent('content-language', 'http-equiv') || 
  ...
  const resolvedFavicon = favicon ? new URL(favicon, baseUrl.href).href : undefined;
...
(async () => {
  try {
    ...
    const { html, status, contentType } = await fetchWebPageWithPuppeteerGhost(url!);
